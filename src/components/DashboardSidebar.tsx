...
/**
 * Fixed-width sidebar with small centered captions under each icon.
 * No toggle/collapse behavior — always visible.
 *
 * To change width, edit SIDEBAR_WIDTH_PX below.
 */
const SIDEBAR_WIDTH_PX = 96; // change this value to desired width (px)
  ...
  return (
    ...
    >
      ...
      <div className="h-16 flex items-center justify-center px-2 border-b border-green-100 flex-shrink-0">
        {/* If you have a logo URL in settings, show it; otherwise show site short name */}
        {settings?.logo_url ? (
          <></>
          // <img
          //   src={settings.logo_url}
      ...
      <div className="flex-1 overflow-y-auto sidebar-scroll py-3">
        <nav className="flex flex-col items-center gap-1">
          ...
          {/* Settings block (collapsible children are shown only as small labels) */}
          <div className="w-full mt-2">
            <div
              className={cn(
                "w-full flex flex-col items-center gap-1 px-1 py-2 transition-colors",
                settingsActive ? "bg-amber-200 border-l-4 border-amber-600" : "hover:bg-amber-100"
              )}
            >
              <div className="w-8 h-8 inline-flex items-center justify-center rounded-md text-amber-900">
                <Settings className="w-5 h-5" />
              </div>
              <span className="text-[10px] text-center leading-[11px] w-full truncate text-amber-900">Settings</span>
            </div>

            {/* children (if you want them visible, show as smaller links) */}
