  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      <div className="flex flex-col flex-1">
        {/* Header */}
        {/* <header className="bg-primary text-primary-foreground px-6 py-3 border-b">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <h1 className="text-lg font-semibold">ETP Store V5.5 Patch 43(Delivery 070)</h1>
              <span className="text-sm opacity-90">COUNTER:3</span>
            </div>
            <h2 className="text-lg font-semibold">Trent Hypermarket Pvt Ltd.</h2>
            <span className="text-sm">SHIFT:1</span>
          </div>
        </header> */}

        {/* Tabs */}
        {/* <div className="flex gap-1 bg-secondary px-4 py-2 border-b">
          {tabs.map((tab) => (
            <Button key={tab} variant={activeTab === tab ? "default" : "ghost"} className={activeTab === tab ? "bg-primary text-primary-foreground" : ""} onClick={() => setActiveTab(tab)}>
              {tab}
            </Button>
          ))}
        </div> */}
